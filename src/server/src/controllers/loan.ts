...
export async function getLoanByIdControllerById(id: string | undefined, res: Response) {
  try {
    ...
    if (!loan) {
      return res.status(404).json({ error: true, errorCode: 'ERR_LOAN_NOT_FOUND', message: 'Empréstimo não encontrado' });
    }
...
export async function deleteLoanController(req: Request, res: Response) {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    return deleteLoanControllerById(id, res);
  } catch (error) {
    return handleError(res, error);
  }
}

export async function deleteLoanControllerById(id: string | undefined, res: Response) {
  try {
    if (!id || typeof id !== 'string') {
      return res.status(400).json({ error: true, errorCode: 'ERR_INVALID_ID', message: 'ID inválido' });
    }
...
export async function getLoansByStudentControllerById(studentId: string | undefined, res: Response) {
  try {
    if (!studentId || typeof studentId !== 'string') {
      return res.status(400).json({ error: true, errorCode: 'ERR_INVALID_STUDENT_ID', message: 'ID do estudante inválido' });
    }
